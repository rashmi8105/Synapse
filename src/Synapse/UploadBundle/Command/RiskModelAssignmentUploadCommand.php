...
class RiskModelAssignmentUploadCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('RiskModelAssignmentUpload:process')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $logger->info(">>>>>>>>>>I am in RiskModelAssignmentUploadCommand");
        $this->output = $output;
        ...
        $modelAssignmentUploadService = $this->getContainer()->get('risk_model_assignment_upload_service');
        $cache = $this->getContainer()->get('synapse_redis_cache');
        
        $logger->info(">>>>>>>>>>I am in RiskModelAssignmentUploadCommand load service");
        
        $fileData = $modelAssignmentUploadService->load($file, $userId);
        $cache->save($cacheKey, [
            UploadConstant::JOBSTATUS => 'loaded',
            UploadConstant::DATA => $fileData
        ]);
        $logger->info(">>>>>>>>>>I am in RiskModelAssignmentUploadCommand process service");
        $jobData = $modelAssignmentUploadService->process($uploadId);
        $jobs = $jobData['jobs'];
        ...
        while (count($jobs) > 0) {
            ...
            foreach ($jobs as $jobNumber => $job) {
                ...
                if ($status === Resque_Job_Status::STATUS_COMPLETE) {
                    ...
                    $jobErrors = $cache->fetch("riskmodelassignment:upload:{$uploadId}:job:{$jobNumber}:errors");
                    ...
                    $logger->info('Riskmodelassignment Status: Total - ' . $totalJobs . ' Completed - ' . $completedJobs . ' Percent Complete - ' . $percentComplete . '%');
        ...
        if ($errorCount) {
            $logger->info(">>>>>>>>>>I am in RiskModelAssignmentUploadCommand generate error CSV");
            $modelAssignmentUploadService->generateErrorCSV($errors);
            sleep(2);
            $cache->save($cacheKey, [
                UploadConstant::JOBSTATUS => UploadConstant::FINISHED
            ]);
        ...
        $logger->info(">>>>>>>>>>I am in RiskModelAssignmentUploadCommand generate dump CSV");
        $modelAssignmentUploadService->generateDumpCSV();
    }
}