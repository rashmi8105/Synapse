...
class RiskVariableUploadCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('riskVariableUpload:process')
            ->setDescription('Process a Risk Variable upload CSV')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $this->getContainer()
            ...
            ->info(">>>>>>>>>>I am in RiskVariableUploadCommand");
        ...
        $this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in RiskVariableUploadCommand -1");
        $errors = [];
        ...
        $this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in RiskVariableUploadCommand -2-" . $file);
        $fileData = $riskVariableUploadService->load($file, $userId);
        $this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in RiskVariableUploadCommand -3");
        $cache->save($cacheKey, [
            UploadConstant::JOBSTATUS => 'loaded',
            UploadConstant::DATA => $fileData
        ]);
        
        $jobData = $riskVariableUploadService->process($uploadId);
        $this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in RiskVariableUploadCommand -4");
        $jobs = $jobData['jobs'];
        ...
        while (count($jobs) > 0) {
            ...
            foreach ($jobs as $jobNumber => $job) {
                ...
                if ($status === Resque_Job_Status::STATUS_COMPLETE) {
                    ...
                    $jobErrors = $cache->fetch("riskvariable.upload:{$uploadId}:job:{$jobNumber}:errors");
                    if ($jobErrors) {
                    ...
                    $logger->info('Risk Varaible Upload Status: Total - ' . $totalJobs . ' Completed - ' . $completedJobs . ' Percent Complete - ' . $percentComplete . '%');
        ...
        if ($errorCount) {
            $riskVariableUploadService->generateErrorCSV($errors);
            sleep(2);
            $cache->save($cacheKey, [
                UploadConstant::JOBSTATUS => UploadConstant::FINISHED
            ]);
