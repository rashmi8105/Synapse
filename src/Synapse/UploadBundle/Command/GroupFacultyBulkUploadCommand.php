...
class GroupFacultyBulkUploadCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $groupUploadService = $this->getContainer()->get('groupfacultybulk_upload_service');
        ...
        while (count($jobs) > 0) {
            ...
            foreach ($jobs as $jobNumber => $job) {
                ...
                if ($status === Resque_Job_Status::STATUS_COMPLETE) {
                    ...
                    $jobErrors = $cache->fetch("groupfacultybulk.upload.{$uploadId}.job.{$jobNumber}.errors");
                    $logger->info("############## ");
                    if ($jobErrors) {
                    ...
                    $logger->info(' Group Faculty Upload Status: Total - ' . $totalJobs . ' Completed - ' . $completedJobs . ' Percent Complete - ' . $percentComplete . '%');
        ...
        $groupUploadService->sendFTPNotification($orgId, 'group_faculty', $errorCount, $downloadFailedLogFile);
