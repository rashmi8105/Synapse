...
class TipUploadCommand extends ContainerAwareCommand
{
    ...
    {	
        $this->setName('tipUpload:process')
            ->setDescription('Process a Tip upload CSV')
    ...
    {	       
        ...
        $this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in TipUploadCommand -1");
        $errors = [];
        ...
        $reportTipUploadService = $this->getContainer()->get('report_tip_upload_service');         
        $cache = $this->getContainer()->get('synapse_redis_cache');        
		$this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in TipUploadCommand -2-" . $file);
        $fileData = $reportTipUploadService->load($file, $userId);
        $this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in TipUploadCommand -3");
        $cache->save($cacheKey, [
            UploadConstant::JOBSTATUS => 'loaded',
            UploadConstant::DATA => $fileData
        ]);
        
        $jobData = $reportTipUploadService->process($uploadId);
        $this->getContainer()
            ->get(UploadConstant::LOGGER)
            ->info(">>>>>>>>>>I am in TipUploadCommand -4");
        $jobs = $jobData['jobs'];		
        ...
        while (count($jobs) > 0) {
            ...
            foreach ($jobs as $jobNumber => $job) {
                ...
                if ($status === Resque_Job_Status::STATUS_COMPLETE) {
                    ...
                    $jobErrors = $cache->fetch("reportTip.upload:{$uploadId}:job:{$jobNumber}:errors");
                    if ($jobErrors) {
                    ...
                    $logger->info('Report Tip upload status: Total - ' . $totalJobs . ' Completed - ' . $completedJobs . ' Percent Complete - ' . $percentComplete . '%');
        ...
        if ($errorCount) {
            $reportTipUploadService->generateErrorCSV($errors);
            sleep(2);
            $cache->save($cacheKey, [
                UploadConstant::JOBSTATUS => UploadConstant::FINISHED
            ]);
        ...
        $reportTipUploadService->generateDumpCSV();
    }
}