...
-- Description: 2.0 with Soft Deletion
-- Function: The work is based on the the first version but take out duplication calculation by bypassing the view
--          of the person_risk_level_calc and its calling view and functions.  Updated to be soft delete compatible
-- ============================================================================================================
USE `synapse`;
DROP PROCEDURE IF EXISTS `risk_calculation_V2`;

DELIMITER $$
USE `synapse`$$
CREATE DEFINER=`synapsemaster`@`%` PROCEDURE `risk_calculation_V2`(deadline TIMESTAMP, chunksize SMALLINT UNSIGNED)
DETERMINISTIC
    ...
    BEGIN
        ...
        WHILE(
            ...
            CREATE TEMPORARY TABLE tmp_person_chunk(org_id INT NOT NULL, person_id INT NOT NULL, queued_at DATETIME DEFAULT NULL);
            ...
            INSERT INTO tmp_person_risk_level_history(person_id, date_captured, risk_model_id, risk_level, risk_score, weighted_value, maximum_weight_value, queued_at)
                SELECT
                    ...
                    PRC.queued_at
                FROM
                    tmp_person_chunk tpc
                    INNER JOIN risk_group_person_history rgph ON tpc.person_id = rgph.person_id
                    INNER JOIN org_risk_group_model orgm
                        ON rgph.risk_group_id = orgm.risk_group_id
                           AND orgm.org_id = tpc.org_id
                    ...
                    LEFT JOIN (SELECT
                                 ...
                                 GROUP BY
                                     ...
                                     tocrv.org_id,
                                     tocrv.queued_at) PRC
                        ON tpc.person_id = PRC.person_id
