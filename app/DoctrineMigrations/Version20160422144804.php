 ...
 */
class Version20160422144804 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
...
left join org_academic_year as oay ON opc.org_academic_year_id = oay.id )
    ...
    SELECT ',@metalist,' FROM (SELECT
        ...
        WHERE  person_id = p.id AND SL.name =  "Transition One" and opss.deleted_at IS NULL LIMIT 1  
		) as TransitiononeReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        on (opss.survey_id = s.id  and s.year_id = "$yearId")
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE  person_id = p.id AND SL.name =  "Check-Up One" and opss.deleted_at IS NULL LIMIT 1
		) as CheckuponeReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        on (opss.survey_id = s.id  and s.year_id = "$yearId")
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE  person_id = p.id AND SL.name =  "Transition Two" and opss.deleted_at IS NULL LIMIT 1
		) as TransitiontwoReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        ON (opss.survey_id = s.id  and s.year_id = "$yearId")
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE  person_id = p.id AND SL.name =  "Check-Up Two" and opss.deleted_at IS NULL LIMIT 1
