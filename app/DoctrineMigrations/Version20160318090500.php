 ...
 */
class Version20160318090500 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
        ...
        update ebi_search SET `query` = 'SELECT SQL_CALC_FOUND_ROWS
                ...
                WHERE
                        ...
                        AND EXISTS(SELECT id FROM
                            academic_update AU WHERE AU.person_id_student = P.id AND AU.final_grade IN (\"C\", \"C-\", \"D+\", \"D\" ,\"D-\",\"F\", \"No Pass\", \"F/No Pass\")
                            AND AU.update_date BETWEEN \"$startDate\" AND \"$endDate\")
                GROUP BY P.id'
                WHERE  `query_key` = 'Final_Grade_Of_C_Or_Below';";
