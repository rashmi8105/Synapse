...
/**
 * Added in new multi-year features into the student data dump. Two new columns
 *
 * Fix for ESPRJ-12443
 *
 */
class Version20161122165338 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
...
left join org_academic_year as oay ON opc.org_academic_year_id = oay.id where opc.organization_id = ',orgId,')
    ...
    distinct
		...
		IFNULL(opsy.is_active, "") AS IsActive,
        IF(opsy.id, 1, "") AS Participating,
...
-- ORDER BY person_id , year_id
 ...
 op.person_id = l.person_id
	 ...
	 left join org_person_student_year as opsy on opsy.person_id = p.id AND oay.id = opsy.org_academic_year_id AND opsy.deleted_at IS NULL AND innie.term_id=""
