 ...
 */
class Version20151026160104 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSQL('insert into org_calc_flags_student_reports(org_id, person_id, created_at, modified_at, calculated_at)
        select d.org_id, d.person_id, NOW(), NOW(), NULL from (
        select org_id, person_id, survey_id, report_id, section_id, element_id, COUNT(*) from report_calculated_values 
        group by org_id, person_id, survey_id, report_id, section_id, element_id HAVING COUNT(*) > 1) as d;');


        $this->addSQL('ALTER IGNORE TABLE `synapse`.`report_calculated_values`
        DROP INDEX `unique_key`,
        ADD UNIQUE INDEX `unique_key` ( `org_id` ASC, `person_id` ASC, `survey_id` ASC, `report_id` ASC, `section_id` ASC, `element_id` ASC);');
