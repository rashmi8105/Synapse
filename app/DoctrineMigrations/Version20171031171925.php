...
/**
 * ESPRJ-11589-ESPRJ-11403-ESPRJ-11645 Soft Deletion Changes to Stored Procedure V2 Risk
 * THIS CREATES THE NEW VERSION 2.0 RISK
 */
class Version20171031171925 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
        ...
        $this->addSql("CREATE DEFINER=`synapsemaster`@`%` PROCEDURE `risk_calculation_V2`(deadline TIMESTAMP, chunksize SMALLINT UNSIGNED)
                            ...
                            BEGIN
                                        ...
                                        #--Carve out a limited chunk to materialize
                                            CREATE TEMPORARY TABLE tmp_person_chunk(person_id INT NOT NULL);
                                        ...
                                        #--Insert IGNORE INTO tmp_org_calculated_risk_variables
                                            INSERT IGNORE INTO tmp_org_calculated_risk_variables (org_id, person_id, risk_variable_id, risk_group_id, risk_model_id, calc_bucket_value, weight, risk_source_value)
                                            SELECT
                                                OCRVV.org_id,
                                                OCRVV.person_id,
                                            ...
                                            FROM
                                                org_calculated_risk_variables_view AS OCRVV
                                                INNER JOIN tmp_person_chunk TPC on OCRVV.person_id = TPC.person_id;
                                        ...
                                        #--Load data in the transition table into person_risk_level_history
                                            INSERT INTO tmp_person_risk_level_history(person_id, date_captured, risk_model_id, risk_level, risk_score, weighted_value, maximum_weight_value)
                                            SELECT
                                                PRC.person_id,
                                                the_ts AS date_captured,
                                                orgm.risk_model_id,
                                                RML.risk_level,
                                                PRC.risk_score,
                                                PRC.RS_Numerator,
                                            ...
                                            FROM
                                                ...
                                                LEFT JOIN risk_model_levels RML ON RML.risk_model_id = orgm.risk_model_id
                                         ...
                                         #--Load data from the transition table into person_risk_level_history
                                            INSERT IGNORE INTO synapse.person_risk_level_history(person_id, date_captured, risk_model_id, risk_level, risk_score, weighted_value, maximum_weight_value)
                                            SELECT
                                                person_id,
                                                date_captured,
                                                risk_model_id,
                                        ...
                                        #-- Set magic dates for blank scores, soft deletion purposefully not taken into account here
                                            UPDATE
                                                org_calc_flags_risk AS OCFR
                                                INNER JOIN
                                                    tmp_person_risk_level_history TPRLH ON TPRLH.person_id = OCFR.person_id
                                            SET
