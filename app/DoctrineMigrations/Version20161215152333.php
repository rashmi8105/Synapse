...
/**
 * Copy and paste from Version20161213171043: fix for merged code against ESPRJ-12675
 *
 * fixed query so the org_person_student_year is not joined against innie table.
 * This allows students to have values in participating and isActive without
 * needed an org_academic_year change. Added 1s and 0s to the participating and
 * IsActive columns.
 */
class Version20161215152333 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
...
left join org_academic_year as oay ON opc.org_academic_year_id = oay.id where opc.organization_id = ',orgId,')
    ...
    SET @metalist = NULL;
    SELECT CONCAT("ExternalId,AuthUsername,Firstname,Lastname,Title,RecordType,StudentPhoto,IsActive,Participating,SurveyCohort,TransitiononeReceiveSurvey,CheckuponeReceiveSurvey,TransitiontwoReceiveSurvey,CheckuptwoReceiveSurvey,YearId,TermId,PrimaryConnect,RiskGroupId,StudentAuthKey,IsPrivacyPolicyAccepted,PrivacyPolicyAcceptedDate,Address1,Address2,City,State,Country,Zip,PrimaryMobile,AlternateMobile,HomePhone,PrimaryEmail,AlternateEmail,PrimaryMobileProvider,AlternateMobileProvider,", Group_concat("`", meta_key, "`"))
    INTO   @metalist
    FROM   listofmetacols AS md;
...
AND innie.person_id IS NULL AND opsy.id IS NULL)
)
