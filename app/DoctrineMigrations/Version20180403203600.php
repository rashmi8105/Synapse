...
/**
 * ESPRJ-13222; updated the student data dump
 */
class Version20180403203600 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
...
FROM
    ...
    WHERE
        u.upload_name = 'student'
            ...
            AND uchdm.deleted_at IS NULL UNION SELECT
        em.meta_key, sort_order, 1
    FROM
        upload u
    ...
    WHERE
        u.upload_name = 'student'
            ...
            AND edt.deleted_at IS NULL
            AND uemchd.deleted_at IS NULL UNION SELECT
        meta_key,
            (SELECT
                    MAX(sort_order)
                FROM
                    upload_ebi_metadata_column_header_download_map) + id,
            2
    FROM
        org_metadata om
    WHERE
...
HEREDOC;
        ...
        $this->addSql($studentDataDumpSql);
