...
/**
 * This migration script is the fix for ESPRJ-10202;
 * The issue was that the previous migration script
 * has the "DELIMITER $$" section which is already
 * added in the addSql php function. This broke the
 * stored procedure it was trying to fix.
 */
class Version20160512192352 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
...
left join org_academic_year as oay ON opc.org_academic_year_id = oay.id )
    ...
    SET @metalist = NULL;
    SELECT CONCAT("ExternalId,AuthUsername,Firstname,Lastname,Title,RecordType,StudentPhoto,IsActive,SurveyCohort,TransitiononeReceiveSurvey,CheckuponeReceiveSurvey,TransitiontwoReceiveSurvey,CheckuptwoReceiveSurvey,YearId,TermId,PrimaryConnect,RiskGroupId,StudentAuthKey,Address1,Address2,City,State,Country,Zip,PrimaryMobile,AlternateMobile,HomePhone,PrimaryEmail,AlternateEmail,PrimaryMobileProvider,AlternateMobileProvider,", Group_concat("`", meta_key, "`"))
    INTO   @metalist
    FROM   listofmetacols AS md;
    ...
    SELECT ',@metalist,' FROM (SELECT
        ...
        IFNULL(opsc.cohort, "") AS SurveyCohort,
        (
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        ON (opss.survey_id = s.id)
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE s.year_id = innie.year_id AND person_id = p.id AND SL.name =  "Transition One" and opss.deleted_at IS NULL LIMIT 1
		) as TransitiononeReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        on (opss.survey_id = s.id)
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE s.year_id = innie.year_id AND  person_id = p.id AND SL.name =  "Check-Up One" and opss.deleted_at IS NULL LIMIT 1
		) as CheckuponeReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        on (opss.survey_id = s.id)
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE s.year_id = innie.year_id AND person_id = p.id AND SL.name =  "Transition Two" and opss.deleted_at IS NULL LIMIT 1
		) as TransitiontwoReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        ON (opss.survey_id = s.id)
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE s.year_id = innie.year_id AND person_id = p.id AND SL.name =  "Check-Up Two" and opss.deleted_at IS NULL LIMIT 1
		) as CheckuptwoReceiveSurvey,
    ...
    from listofpeeps l
     ...
     left join risk_group_person_history as rh on rh.person_id = (SELECT rgh.person_id FROM risk_group_person_history as rgh WHERE rgh.person_id = p.id ORDER BY rgh.assignment_date DESC LIMIT 1)
	  ...
	  oay.deleted_at IS NULL and
	  opsc.deleted_at IS NULL and
	  pinfo.deleted_at IS NULL and
	  pci.deleted_at IS NULL and
	  pcinfo.deleted_at IS NULL and
