...
/**
 * Migration script to create "StudentIssuesCalculation" stored procedure that creates "student_issues" temporary table.
 *
 * ESPRJ-15629
 */
class Version20170803112200 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        $this->abortIf($this->connection->getDatabasePlatform()->getName() != "mysql", "Migration can only be executed safely on 'mysql'.");
        $this->addSql("DROP procedure IF EXISTS StudentIssuesCalculation;");
        $this->addSql("
			...
			FROM (
				...
				WHERE
					faculty_id = p_facultyid) AS ofspm
			JOIN org_person_student_survey_link opssl ON opssl.person_id = ofspm.student_id
					AND opssl.deleted_at IS NULL AND opssl.survey_id = p_surveyid
			...
			WHERE (SELECT 1 FROM wess_link wl WHERE wl.survey_id = p_surveyid
    ...
    {
        $this->abortIf($this->connection->getDatabasePlatform()->getName() != "mysql", "Migration can only be executed safely on 'mysql'.");
        $this->addSql("DROP procedure StudentIssuesCalculation");
    }
}
