...
/**
 * Migration file to edit the student Datadump to get receive survey cols from org_person_student_survey table 
 */
class Version20160405084740 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        //dynamcally generating the yearid
        ...
        if ($currentMonth > 8) {
        	...
        	$yearId = $currentYear . substr($nextYear, - 2);
        } else {
        	$prevYear = $currentYear - 1;
        	$yearId = $prevYear . substr($currentYear, - 2);
        }
...
left join org_academic_year as oay ON opc.org_academic_year_id = oay.id )
    ...
    SELECT ',@metalist,' FROM (SELECT
        ...
        ON (opss.survey_id = s.id  AND s.year_id = "$yearId")
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE  person_id = p.id AND SL.name =  "Mapworks: Transition One" and opss.deleted_at IS NULL LIMIT 1  
		) as TransitiononeReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        on (opss.survey_id = s.id  and s.year_id = "$yearId")
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE  person_id = p.id AND SL.name =  "Mapworks: Check-Up One" and opss.deleted_at IS NULL LIMIT 1
		) as CheckuponeReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        on (opss.survey_id = s.id  and s.year_id = "$yearId")
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE  person_id = p.id AND SL.name =  "Mapworks: Transition Two" and opss.deleted_at IS NULL LIMIT 1
		) as TransitiontwoReceiveSurvey,
		(
        SELECT receive_survey  FROM org_person_student_survey as opss INNER JOIN survey as s
        ON (opss.survey_id = s.id  and s.year_id = "$yearId")
        INNER JOIN survey_lang as SL on s.id = SL.survey_id
        WHERE  person_id = p.id AND SL.name =  "Mapworks: Check-Up Two" and opss.deleted_at IS NULL LIMIT 1
